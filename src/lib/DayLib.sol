...
library DayLib {
     ...
     * @return day The number of days since game start (uint16)
     ...
     */
    function timestampToDay(uint64 timestamp) internal pure returns (uint16 day) {
        unchecked {
            ...
            day = uint16(secondsSinceStart / 1 days);
        }
    }
     ...
     */
    function getCurrentDay() internal view returns (uint16 day) {
        unchecked {
            // Get current timestamp and convert to uint64
            uint64 currentTimestamp = uint64(block.timestamp);

            // Convert to day using the same logic
            day = timestampToDay(currentTimestamp);
        }
    }
