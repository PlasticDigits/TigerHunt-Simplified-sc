...
import {PlayerEntity, TargetEntity, EntityLib, EntityKey} from "./entity/EntityLib.sol";
...
contract LocationController is AccessManaged {
    ...
    bytes32 public constant ENTITY_NFT_AT_LOCATION_PARTIAL = keccak256("ENTITY_NFT_AT_LOCATION_PARTIAL");
    bytes32 public constant ENTITY_ID_AT_LOCATION_PARTIAL = keccak256("ENTITY_ID_AT_LOCATION_PARTIAL");
    ...
    function setLocation(PlayerEntity calldata playerEntity, PlayerLocation calldata playerLocation)
        external
        restricted
    {
        ...
        DatastoreSetUint256 entityIdOldLocationSet = DATASTORE_SET_WRAPPER.DATASTORE_SET_UINT256();

        // Remove the entity ID from the old location
        entityIdOldLocationSet.remove(
            getEntityIdAtLocationSetKey(oldWorld, oldTile, playerEntity.playerNft), playerEntity.playerNftId
        );
        ...
        if (playerLocation.world != IWorld(address(0))) {
            ...
            DATASTORE_SET_WRAPPER.DATASTORE_SET_UINT256().add(
                getEntityIdAtLocationSetKey(playerLocation.world, playerLocation.tile, playerEntity.playerNft),
                playerEntity.playerNftId
            );
            // Add the entityNFT to the new location
            DATASTORE_SET_WRAPPER.DATASTORE_SET_ADDRESS().add(
                getEntityNftAtLocationSetKey(playerLocation.world, playerLocation.tile), address(playerEntity.playerNft)
            );
        }
        ...
        if (
            entityIdOldLocationSet.length(
                address(this), getEntityIdAtLocationSetKey(oldWorld, oldTile, playerEntity.playerNft)
            ) == 0
        ) {
            DATASTORE_SET_WRAPPER.DATASTORE_SET_ADDRESS().remove(
                getEntityNftAtLocationSetKey(oldWorld, oldTile), address(playerEntity.playerNft)
            );
        }
    ...
    function getLocation(PlayerEntity calldata playerEntity) external view returns (PlayerLocation memory) {
        return playerLocations[playerEntity.key()];
    }

    function isPlayerAndTargetInSameLocation(PlayerEntity calldata playerEntity, TargetEntity calldata targetEntity)
        public
        view
        returns (bool)
    {
        ...
        return playerLocations[playerKey].world == playerLocations[targetKey].world
            && TileID.unwrap(playerLocations[playerKey].tile) == TileID.unwrap(playerLocations[targetKey].tile);
    }

    function getEntityNftAtLocationSetKey(IWorld world, TileID tile) public pure returns (DatastoreSetIdAddress) {
        return DatastoreSetIdAddress.wrap(keccak256(abi.encode(ENTITY_NFT_AT_LOCATION_PARTIAL, world, tile)));
    }

    function getEntityIdAtLocationSetKey(IWorld world, TileID tile, IERC721 entityNft)
        public
        pure
        returns (DatastoreSetIdUint256)
    {
        return DatastoreSetIdUint256.wrap(keccak256(abi.encode(ENTITY_ID_AT_LOCATION_PARTIAL, world, tile, entityNft)));
    }
}
